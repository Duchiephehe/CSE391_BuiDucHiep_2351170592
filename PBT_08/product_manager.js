...
// Dùng mảng copy [...products] để không làm thay đổi mảng gốc (best practice)
...
function cheapestByCategory(products) {
    return products.reduce((acc, curr) => {
        // Nếu category này chưa có trong acc, HOẶC giá của sp hiện tại rẻ hơn sp đã lưu
...
function formatProductList(products) {
    return products.map(p => ({
        name: p.name,
        // Dùng toLocaleString('vi-VN') để phân cách hàng nghìn bằng dấu chấm
...
function averageRating(products) {
    ...
    // Trả về trung bình, làm tròn 1 chữ số thập phân
    return Number((sum / products.length).toFixed(1));
}
...
// Test (chạy các test theo yêu cầu):
